/* Current regression entry point, as of Feature 4.5.12.
 *
 * Delegates to Feature 4.5.11's runner UNCHANGED — which in turn delegates to 4.5.10's, and so on
 * down to 4.5.2's — and adds this release's own suite, reporting one combined total. When 4.5.12
 * is rolled back this file goes with its folder and 4.5.11's runner is the entry point again.
 *
...
const previous = path.join(repo,
  'Versions/PART I — Phase 4.5.11 Seven Fortunes Curse/qa/current-suite-runner.js');
const own = path.join(__dirname, 'disadv-bishamon-harness.js');
...
const previousOutput = runOne('Feature 4.5.11 suite set', previous, [sheet]);
const trailer = /COMBINED (\d+)\/(\d+) checks passed/.exec(previousOutput);
if (trailer) { passed += Number(trailer[1]); total += Number(trailer[2]); }
else { console.log('FAIL the 4.5.11 runner produced no combined total'); failures += 1; }

const ownOutput = runOne('Feature 4.5.12 Bishamon', own, [sheet]);
const ownCounts = /(\d+)\/(\d+) checks passed/.exec(ownOutput);
if (ownCounts) { passed += Number(ownCounts[1]); total += Number(ownCounts[2]); }
else { console.log('FAIL the 4.5.12 harness produced no count'); failures += 1; }
...
console.log(`\nCOMBINED ${passed}/${total} checks passed across every retained suite plus Feature 4.5.12`);
