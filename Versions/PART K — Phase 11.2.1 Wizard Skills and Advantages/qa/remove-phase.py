#!/usr/bin/env python3
"""Surgically remove PART K Phase 11.2.1 (Wizard Skills and Advantages) from a SCRATCH COPY of the
Phase 0 tree.
...
lines, and the wizard2-seam block in 210-test-seam-and-init.js are removed. Phase 11.2's wizard
stays, back to its six steps. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 11.2.1"
FRAGMENTS = (
    "src/sheet/209.995-feat-wizard-skills-advantages.js",
    "src/css/59.995-feat-wizard-skills-advantages.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("wizard2-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "9244163ea532004707a2c8d5c19bf8cde4ecc0d5a39f3731e7ea3d51ba6a6ec0"
PRE_RELEASE_BYTES = 2985043
PRE_RELEASE_COMMIT = "5c50e87"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
# (?![.\d]) so a later 11.2.x marker is never mistaken for this one; Phase 11's own "PART K PHASE
# 11" cannot match either, because this pattern needs the ".2".
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+11\.2\.1(?![.\d])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 11\.2\.1 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END WIZARD1121 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+WIZARD1121\b", re.I)
SURFACE_RE = re.compile(r"\b(?:WIZARD_SKILLS_ADV_ENABLED|CW1121|cw1121[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("wizard2-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END WIZARD1121")
