...
SPEC = importlib.util.spec_from_file_location("wizard_skills_adv_removal", Path(__file__).with_name("remove-phase.py"))
...
def blk(slug, body="  owned();\n"):
    return f"  // PART K PHASE 11.2.1 BEGIN {slug}\n{body}  // END WIZARD1121 {slug}\n"
...
class Blocks(unittest.TestCase):
    ...
    def test_orphan_mismatched_unclosed_refused(self):
        for text in [seam("  // END WIZARD1121 wizard2-seam\n"),
                     seam().replace("END WIZARD1121 wizard2-seam", "END WIZARD1121 other"),
                     seam().replace("  // END WIZARD1121 wizard2-seam\n", "")]:
    ...
    def test_later_point_release_block_is_not_ours(self):
        # A future Phase 11.2.1.1 block must survive untouched, and must not be mistaken for ours.
        later = "  // PART K PHASE 11.2.1.1 BEGIN later\n  later();\n  // END LATER11211 later\n"
    ...
    def test_partial_own_marker_outside_blocks_refused(self):
        for extra in ["  // PART K PHASE 11.2.1 stray\n", "  // END WIZARD1121\n"]:
...
class Scratch(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-cw1121-fixture-")
    ...
    def test_surface_left_in_a_retained_file_refused(self):
        fixture(self.root, shared=seam("  if (typeof CW1121 === 'object') CW1121.addSkill();\n"))
        before = snapshot(self.root)
        with self.assertRaises(R.RemovalError):
            R.remove(self.root)
        self.assertEqual(snapshot(self.root), before)
...
class LiveTree(unittest.TestCase):
    ...
    def test_live_tree_removes_to_the_pre_release_build(self):
        with tempfile.TemporaryDirectory(prefix="l5r-cw1121-live-") as work:
