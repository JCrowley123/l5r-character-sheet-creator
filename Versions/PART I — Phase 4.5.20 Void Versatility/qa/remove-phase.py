#!/usr/bin/env python3
"""Surgically remove Void Versatility from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact void-versatility-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.20"
FRAGMENTS = (
    "src/sheet/209.9297-feat-adv-void-versatility.js",
    "src/css/59.96-adv-void-versatility.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("void-versatility-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "2b69794dcd3bf9d70b8a1676a4f9cb734cf1f4b0ecf7d40b0396d0bae8ef0c3e"
PRE_RELEASE_BYTES = 2798717
PRE_RELEASE_COMMIT = "46b15b41a199f2f227f39dd83b1a5ab49155bc67"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.20(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.20 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END VV4520 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+VV4520\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_VOID_VERSATILITY_ENABLED|VV4520|vv4520[A-Za-z0-9_-]*|voidVersatilityRing)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("void-versatility-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END VV4520")
