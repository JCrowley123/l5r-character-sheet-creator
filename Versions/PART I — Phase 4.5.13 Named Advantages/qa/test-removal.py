...
SPEC = importlib.util.spec_from_file_location("named_removal", Path(__file__).with_name("remove-phase.py"))
R = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(R)
SLUG = "named-advantages-seam"
SHARED = "src/sheet/210-test-seam-and-init.js"


def block(body="  owned();\n", slug=SLUG, eol="\n"):
    return (f"  before();{eol}  // {R.MARKER} BEGIN {slug}{eol}"
            f"{body}  // END NAMED4513 {slug}{eol}  after();{eol}")
...
def fixture(root, eol="\n"):
    ...
    write(root, R.FRAGMENTS[0], f"// {R.MARKER} named entries{eol}const N4513 = {{}};{eol}")
    write(root, R.FRAGMENTS[1], f"/* {R.MARKER} styles */{eol}.named4513-badge {{ color: red; }}{eol}")
...
class BlockFixtures(unittest.TestCase):
    ...
    def test_mismatched_slug_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "mismatched"):
            R.strip_owned_blocks(block().replace("END NAMED4513 " + SLUG, "END NAMED4513 wrong"), "fixture")

    def test_orphan_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "orphan"):
            R.strip_owned_blocks("// END NAMED4513 named-advantages-seam\n", "fixture")

    def test_missing_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "unclosed"):
            R.strip_owned_blocks(block().replace("  // END NAMED4513 " + SLUG + "\n", ""), "fixture")

    def test_partial_marker_refused(self):
        for source in [block().replace("BEGIN", "BEG"), block().replace("END NAMED4513", "END NAMED451"),
    ...
    def test_partial_end_inside_block_refused(self):
        for partial in ["// END NAMED4513", "// end named4513 wrong", "/* END NAMED4513 wrong */"]:
    ...
    def test_foreign_phase_inside_refused(self):
        for marker in ["PART I FEATURE 4.5.12", "PART J PHASE 8", "Part G Phase 3's preview", "BUGFIX"]:
    ...
    def test_later_phases_outside_block_preserved(self):
        later = "// PART I FEATURE 4.5.130 later phase\nkeepLater();\n// BUGFIX other fix\n"
...
class ScratchFixtures(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-named-removal-fixture-")
    ...
    def test_unmanifested_leak_refuses_before_writes(self):
        for token in ["N4513", "ADV_NAMED_ENTRIES_ENABLED", "named4513-badge", "namedEntry4513"]:
...
class ReadOnlyGuardFixtures(unittest.TestCase):
    ...
    def test_missing_live_tree_refuses_blind_operation(self):
        with tempfile.TemporaryDirectory(prefix="l5r-named-guard-") as directory:
