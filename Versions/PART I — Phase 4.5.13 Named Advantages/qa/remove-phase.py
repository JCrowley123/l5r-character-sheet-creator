#!/usr/bin/env python3
"""Surgically remove named Advantages from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact named-advantages-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.13"
FRAGMENTS = (
    "src/sheet/209.925-feat-adv-named.js",
    "src/css/59.8-adv-named.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("named-advantages-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "c7063f52269c306fcd6596e2103518ff27d4a57b01b0db031115dbb59c58ca15"
PRE_RELEASE_BYTES = 2689172
PRE_RELEASE_COMMIT = "6005faecac2e5e671d9c4a2bb45f24ba60044db7"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.13(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.13 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END NAMED4513 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+NAMED4513\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_NAMED_ENTRIES_ENABLED|N4513|NAMED4513|namedEntry4513|named4513[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("named-advantages-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END NAMED4513")
