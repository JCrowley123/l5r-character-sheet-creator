#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12.2 Play Mode Identity (part 3) from a SCRATCH COPY of the Phase 0 tree.
...
manifest lines, and the modes-identity-seam block in 210-test-seam-and-init.js are removed. The
Identity tab goes back to editing in both modes. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12.2"
FRAGMENTS = (
    "src/sheet/209.9998-feat-modes-identity.js",
    "src/css/59.999-feat-modes-identity.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-identity-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "d6c0c79b2e42cef57ce2a061420b229bb482e39d67af35beb3b4a3066d158bd3"
PRE_RELEASE_BYTES = 3085178
PRE_RELEASE_COMMIT = "a3aa5f4"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12\.2(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12\.2 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES122 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES122\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES122_ENABLED|MODES122)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-identity-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES122")
