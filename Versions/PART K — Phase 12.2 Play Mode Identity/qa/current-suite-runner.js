...
const previous = path.resolve(__dirname, '../../PART K — Phase 12.1 Play Mode Clan and School/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Phase 12\.1 \(part 2\) Clan and School/],
  [path.join(__dirname, 'identity-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 12.2 (part 3) Identity`);
