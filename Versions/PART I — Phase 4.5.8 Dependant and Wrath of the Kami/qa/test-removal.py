#!/usr/bin/env python3
"""Positive and negative fixtures for Feature 4.58's removal tool.
...
class BlockTests(unittest.TestCase):
    def wrap(self, body, slug="dependant-wrath-seam", eol="\n"):
        return (f"  before();{eol}"
                f"  // {MARKER} BEGIN {slug}{eol}"
                f"{body}"
                f"  // END AGREED458 {slug}{eol}"
                f"  after();")
    ...
    def test_owned_block_is_cut_and_neighbours_survive_exactly(self):
        ...
        self.assertEqual(removed, ["dependant-wrath-seam"])
    ...
    def test_the_sibling_release_marker_is_left_alone(self):
        # "PART I FEATURE 4.5" is now a prefix of SIX markers, and 4.57's unlucky-seam sits
        ...
        source = ("  // PART I FEATURE 4.53 BEGIN repairs-seam\n  a();\n"
                  ...
                  f"  // {MARKER} BEGIN dependant-wrath-seam\n  ours();\n"
...
class RefusalTests(unittest.TestCase):
    def test_foreign_marker_inside_our_block_is_refused(self):
        source = (f"  // {MARKER} BEGIN dependant-wrath-seam\n  // PART J PHASE 5\n  theirs();\n"
                  "  // END AGREED458 dependant-wrath-seam\n")
    ...
    def test_orphan_end_delimiter_is_refused(self):
        with self.assertRaises(REMOVAL.RemovalError) as caught:
            REMOVAL.strip_owned_blocks("  // END AGREED458 dependant-wrath-seam\n", "shared.js")
...
class ManifestTests(unittest.TestCase):
    ...
    def manifest(self, js=True, css=True, sha="deadbeef"):
        entries = ['    { "file": "src/css/57-disadv-nemesis.css",\n',
                   '      "note": "4.522" },\n']
        if css:
            entries += ['    { "file": "src/css/59.3-disadv-dependant-wrath.css",\n',
                        '      "note": "4.55 css" },\n']
        entries += ['    { "file": "src/sheet/209.89-feat-adv-config-repairs.js",\n',
                    '      "note": "4.53" },\n']
        if js:
            entries += ['    { "file": "src/sheet/209.94-feat-disadv-dependant-wrath.js",\n',
                        '      "note": "4.55 js" },\n']
    ...
    def test_both_entries_are_cut_and_the_rest_is_byte_identical(self):
        ...
        self.assertNotIn("src/sheet/209.94-feat-disadv-dependant-wrath.js", files)
        self.assertNotIn("src/css/59.3-disadv-dependant-wrath.css", files)
...
class RecordedTargetTests(unittest.TestCase):
    def test_pre_release_build_is_the_one_this_release_was_added_to(self):
        ...
        self.assertEqual(REMOVAL.PRE_RELEASE_BYTES, 2562091)
