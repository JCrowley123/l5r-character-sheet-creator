#!/usr/bin/env python3
"""Remove Part I Feature 4.58 (Dependant and Wrath of the Kami) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha ad856e50...
...
Same safety model as Feature 4.55's, 4.56's and 4.57's removers, and for the same reason: explicit
...
MARKER = "PART I FEATURE 4.58"
FRAGMENTS = (
    "src/sheet/209.94-feat-disadv-dependant-wrath.js",
    "src/css/59.3-disadv-dependant-wrath.css",
)
SHARED_FILES = ("src/sheet/210-test-seam-and-init.js",)
PRE_RELEASE_SHA = "ad856e506e35121951923e28c907427c273eacf14a490db5d42ad446b2c3f0cf"
PRE_RELEASE_BYTES = 2562091
# The Feature 4.57 build this release was added to, tooltip correction included. Removing
# 4.58 restores exactly that.
...
END_RE = re.compile(r"\bEND AGREED458\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END AGREED458")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END AGREED458 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
