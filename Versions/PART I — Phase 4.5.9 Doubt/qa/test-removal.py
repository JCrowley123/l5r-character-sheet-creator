#!/usr/bin/env python3
"""Positive and negative fixtures for Feature 4.59's removal tool.
...
class BlockTests(unittest.TestCase):
    def wrap(self, body, slug="doubt-seam", eol="\n"):
        return (f"  before();{eol}"
                f"  // {MARKER} BEGIN {slug}{eol}"
                f"{body}"
                f"  // END DOUBT459 {slug}{eol}"
                f"  after();")
    ...
    def test_owned_block_is_cut_and_neighbours_survive_exactly(self):
        ...
        self.assertEqual(removed, ["doubt-seam"])
    ...
    def test_the_sibling_release_marker_is_left_alone(self):
        # "PART I FEATURE 4.5" is now a prefix of SEVEN markers, and 4.58's dependant-wrath-seam sits
...
class RefusalTests(unittest.TestCase):
    def test_foreign_marker_inside_our_block_is_refused(self):
        source = (f"  // {MARKER} BEGIN doubt-seam\n  // PART J PHASE 5\n  theirs();\n"
                  "  // END DOUBT459 doubt-seam\n")
    ...
    def test_orphan_end_delimiter_is_refused(self):
        with self.assertRaises(REMOVAL.RemovalError) as caught:
            REMOVAL.strip_owned_blocks("  // END DOUBT459 doubt-seam\n", "shared.js")
...
class ManifestTests(unittest.TestCase):
    ...
    def test_both_entries_are_cut_and_the_rest_is_byte_identical(self):
        ...
        self.assertNotIn("src/sheet/209.95-feat-disadv-doubt.js", files)
        self.assertNotIn("src/css/59.4-disadv-doubt.css", files)
...
class RecordedTargetTests(unittest.TestCase):
    def test_pre_release_build_is_the_one_this_release_was_added_to(self):
        ...
        self.assertEqual(REMOVAL.PRE_RELEASE_BYTES, 2572964)
