#!/usr/bin/env python3
"""Remove Part I Feature 4.59 (Doubt) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha e61137ad...
...
Same safety model as Features 4.55 through 4.58's removers, and for the same reason: explicit
...
MARKER = "PART I FEATURE 4.59"
FRAGMENTS = (
    "src/sheet/209.95-feat-disadv-doubt.js",
    "src/css/59.4-disadv-doubt.css",
)
SHARED_FILES = ("src/sheet/210-test-seam-and-init.js",)
PRE_RELEASE_SHA = "e61137adcd8d6b891c17c69a7f498d936dbc36eabf5531e77fa4b36a47496104"
PRE_RELEASE_BYTES = 2572964
# The Feature 4.58 build this release was added to. Removing 4.59 restores exactly that.
...
END_RE = re.compile(r"\bEND DOUBT459\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END DOUBT459")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END DOUBT459 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
