...
const previous = path.resolve(__dirname, '../../PART K — Phase 11.2.2 Wizard Free Choices Spells and Kiho/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Phase 11.2.2 Wizard Free Choices, Spells and Kiho/],
  [path.join(__dirname, 'wizard4-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 11.2.3 Wizard Starting Spells`);
