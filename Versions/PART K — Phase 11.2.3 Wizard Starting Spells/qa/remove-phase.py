#!/usr/bin/env python3
"""Surgically remove PART K Phase 11.2.3 (Wizard Starting Spells) from a SCRATCH COPY of the
Phase 0 tree.
...
lines, and the wizard4-seam block in 210-test-seam-and-init.js are removed. Phases 11.2 to
11.2.2 stay; the Spells step goes back to 11.2.2's generic picker. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 11.2.3"
FRAGMENTS = (
    "src/sheet/209.997-feat-wizard-starting-spells.js",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("wizard4-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "a2d148ee90ee2e877fc74c100e1766ad68620193dbae389763a6212cdec12953"
PRE_RELEASE_BYTES = 3030719
PRE_RELEASE_COMMIT = "8c9b9eb"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
# (?![.\d]) so a later 11.2.3.x marker is never mistaken for this one; the earlier 11.2.x markers
# cannot match either, because this pattern needs the ".2.3".
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+11\.2\.3(?![.\d])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 11\.2\.3 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END WIZARD1123 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+WIZARD1123\b", re.I)
SURFACE_RE = re.compile(r"\b(?:WIZARD_STARTING_SPELLS_ENABLED|STARTING_SPELLS|CW1123|cw1123[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("wizard4-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END WIZARD1123")
