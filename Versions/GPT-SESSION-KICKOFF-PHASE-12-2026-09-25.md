...
`CLAUDE-SESSION-KICKOFF-PHASE-12-2026-09-25.md`; the project's rules are the same whichever assistant
does the work.
...
**Part 5 (Skills) was started straight after this file was written**, on branch
`claude/phase-12-part-5-2026-09-25`. Check that branch and the ledger's current update for whether
it finished, was tested, and was merged. **Do not assume it is on `main`.**
...
- **`Versions/CLAUDE.md` is the project's rulebook for every assistant**, not only Claude. Follow it
  exactly: folder convention, surgical removal, delimited markers, kill switches, harnesses that are
  proven able to fail, byte-identical removal, one commit per release.
- **You cannot publish the Claude ledger artifact.** Update `BUILD-LEDGER.md` and `BUILD-LEDGER.html`
  in the repository; the owner or a Claude session republishes the artifact.
