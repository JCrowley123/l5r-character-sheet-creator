#!/usr/bin/env python3
"""Surgically remove Touch of the Spirit Realms from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact spirit-realms-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.24"
FRAGMENTS = (
    "src/sheet/209.92992-feat-adv-spirit-realms.js",
    "src/css/59.991-adv-spirit-realms.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("spirit-realms-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "4d112320eb4c1e9937a280b1e7d3f25ef1d5aff49a5a938e80b351fa329ccac7"
PRE_RELEASE_BYTES = 2888142
PRE_RELEASE_COMMIT = "94235855fde49542fbac785f8f2534624051afbd"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.24(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.24 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END TR4524 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+TR4524\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_SPIRIT_REALMS_ENABLED|TR4524|tr4524[A-Za-z0-9_-]*|spiritRealmTouch)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("spirit-realms-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END TR4524")
