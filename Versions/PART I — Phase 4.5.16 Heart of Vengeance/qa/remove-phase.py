#!/usr/bin/env python3
"""Surgically remove Heart of Vengeance from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact heart-vengeance-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.16"
FRAGMENTS = (
    "src/sheet/209.928-feat-adv-heart-vengeance.js",
    "src/css/59.92-adv-heart-vengeance.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("heart-vengeance-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "1d8aa345b8c054cca212bd57a57acb49cb593bf4b7805cec0438a171537a70e4"
PRE_RELEASE_BYTES = 2739496
PRE_RELEASE_COMMIT = "45d0f489081de3c5740c60b74afb0714396b6a6d"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.16(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.16 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END HV4516 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+HV4516\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_HEART_VENGEANCE_ENABLED|HV4516|hv4516[A-Za-z0-9_-]*|factionPick)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("heart-vengeance-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END HV4516")
