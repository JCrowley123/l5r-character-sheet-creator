#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12.4 Play Mode Skills (part 5) from a SCRATCH COPY of the Phase 0 tree.
...
manifest lines, and the modes-skills-seam block in 210-test-seam-and-init.js are removed. The
Skills tab goes back to editing in both modes. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12.4"
FRAGMENTS = (
    "src/sheet/209.99991-feat-modes-skills.js",
    "src/css/59.9992-feat-modes-skills.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-skills-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "f3174bba1fa485221579882528163e7de22a666e43b79cb8e6d17c7f73371798"
PRE_RELEASE_BYTES = 3089890
PRE_RELEASE_COMMIT = "6184ee5"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12\.4(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12\.4 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES124 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES124\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES124_ENABLED|MODES124)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-skills-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES124")
