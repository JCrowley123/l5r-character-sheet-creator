#!/usr/bin/env python3
"""Surgically remove Dark Paragon from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact dark-paragon-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.23"
FRAGMENTS = (
    "src/sheet/209.92991-feat-adv-dark-paragon.js",
    "src/css/59.99-adv-dark-paragon.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("dark-paragon-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "985fdeeeb4888e2d62195b5182481c5f6da053418f0737b00bbacd9c9d8b9a3f"
PRE_RELEASE_BYTES = 2854260
PRE_RELEASE_COMMIT = "862dfbf316d8d05532e9b59d26dac9ddb844fb66"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.23(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.23 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END DP4523 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+DP4523\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_DARK_PARAGON_ENABLED|DP4523|dp4523[A-Za-z0-9_-]*|darkParagon)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("dark-paragon-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END DP4523")
