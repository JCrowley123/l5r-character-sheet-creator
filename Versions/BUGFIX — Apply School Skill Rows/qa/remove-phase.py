#!/usr/bin/env python3
"""Surgically remove BUGFIX — Apply School Skill Rows from a SCRATCH COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY [--dry-run] [--expect-sha SHA256]

Preflights the complete operation before writing. Only the owned fragment, its manifest line, the
two school-skill-choice blocks in 080-identity-build-ui.js and the school-skill-rows-seam block in
210-test-seam-and-init.js are removed. Apply School goes back to its old rows (no Trait for
"Family: Subject" Skills, the three misnamed Skills, the placeholder rows). Later features are
preserved; the resulting manifest hash is calculated from the
...
MARKER = "BUGFIX"  # MARKER_RE captures the bare word; the long form is BUGFIX SCHOOLSKILLROWS
FRAGMENTS = (
    "src/sheet/209.9995-bugfix-school-skill-rows.js",
)
SHARED_BLOCKS = {
    "src/sheet/080-identity-build-ui.js": ("school-skill-choice-state", "school-skill-choice-token"),
    "src/sheet/210-test-seam-and-init.js": ("school-skill-rows-seam",),
}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "738c7ccfa3bdf67a6ca160371508fe22521cac845b883119b95a5a4e9490958a"
PRE_RELEASE_BYTES = 3062010
PRE_RELEASE_COMMIT = "322d9e6"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bBUGFIX\s+SCHOOLSKILLROWS\b", re.I)
BEGIN_RE = re.compile(r"^\s*// BUGFIX SCHOOLSKILLROWS BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END SCHOOLSKILLROWS ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+SCHOOLSKILLROWS\b", re.I)
SURFACE_RE = re.compile(r"\b(?:SCHOOL_SKILL_ROWS_FIX_ENABLED|SCHOOL_SKILL_ROWS|schoolSkillRowsReader|schoolSkillRowsPrevious[A-Za-z]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("school-skill-rows-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END SCHOOLSKILLROWS")
