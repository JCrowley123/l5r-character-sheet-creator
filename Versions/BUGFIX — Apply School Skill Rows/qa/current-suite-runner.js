...
const previous = path.resolve(__dirname, '../../BUGFIX — Import File Picker Filter/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus the Import File Picker Filter bugfix/],
  [path.join(__dirname, 'school-skill-rows-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus the Apply School Skill Rows bugfix`);
