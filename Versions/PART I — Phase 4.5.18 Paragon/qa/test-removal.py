...
SPEC = importlib.util.spec_from_file_location("paragon_removal", Path(__file__).with_name("remove-phase.py"))
...
def block(body="  owned();\n", slug=SLUG, eol="\n"):
    return (f"  before();{eol}  // {R.MARKER} BEGIN {slug}{eol}"
            f"{body}  // END P4518 {slug}{eol}  after();{eol}")
...
def fixture(root, eol="\n"):
    ...
    write(root, R.FRAGMENTS[0], f"// {R.MARKER} paragon{eol}const P4518 = {{}};{eol}")
    write(root, R.FRAGMENTS[1], f"/* {R.MARKER} styles */{eol}.p4518-badge {{ color: red; }}{eol}")
...
class BlockFixtures(unittest.TestCase):
    ...
    def test_mismatched_slug_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "mismatched"):
            R.strip_owned_blocks(block().replace("END P4518 " + SLUG, "END P4518 wrong"), "fixture")

    def test_orphan_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "orphan"):
            R.strip_owned_blocks("// END P4518 paragon-seam\n", "fixture")

    def test_missing_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "unclosed"):
            R.strip_owned_blocks(block().replace("  // END P4518 " + SLUG + "\n", ""), "fixture")

    def test_partial_marker_refused(self):
        for source in [block().replace("BEGIN", "BEG"), block().replace("END P4518", "END W451"),
    ...
    def test_partial_end_inside_block_refused(self):
        for partial in ["// END P4518", "// end p4518 wrong", "/* END P4518 wrong */"]:
    ...
    def test_foreign_phase_inside_refused(self):
        for marker in ["PART I FEATURE 4.5.17", "PART J PHASE 8", "Part G Phase 3's preview", "BUGFIX"]:
    ...
    def test_later_phases_outside_block_preserved(self):
        later = "// PART I FEATURE 4.5.180 later phase\nkeepLater();\n// PART I FEATURE 4.5.1 earlier\n// BUGFIX other fix\n"
...
class ScratchFixtures(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-p-removal-fixture-")
    ...
    def test_unmanifested_leak_refuses_before_writes(self):
        for token in ["P4518", "ADV_PARAGON_ENABLED", "p4518-badge", "p4518Tenet", "p4518", "paragonTenet"]:
...
class ReadOnlyGuardFixtures(unittest.TestCase):
    ...
    def test_missing_live_tree_refuses_blind_operation(self):
        with tempfile.TemporaryDirectory(prefix="l5r-p-guard-") as directory:
