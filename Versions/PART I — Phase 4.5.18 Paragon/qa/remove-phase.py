#!/usr/bin/env python3
"""Surgically remove Paragon from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact paragon-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.18"
FRAGMENTS = (
    "src/sheet/209.9295-feat-adv-paragon.js",
    "src/css/59.94-adv-paragon.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("paragon-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "be9076cfbebc3fac31641a03fab82508b35a7c06caf961e347e546151930115a"
PRE_RELEASE_BYTES = 2767985
PRE_RELEASE_COMMIT = "bfd82eef4fa38b516bfea36cd38a031adf4bccbc"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.18(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.18 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END P4518 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+P4518\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_PARAGON_ENABLED|P4518|p4518[A-Za-z0-9_-]*|paragonTenet)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("paragon-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END P4518")
