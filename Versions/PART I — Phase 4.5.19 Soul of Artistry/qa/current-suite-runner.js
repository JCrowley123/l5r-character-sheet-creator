...
const previous = path.resolve(__dirname, '../../PART I — Phase 4.5.18 Paragon/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Paragon/],
  [path.join(__dirname, 'soul-artistry-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Soul of Artistry`);
