#!/usr/bin/env python3
"""Surgically remove Soul of Artistry from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact soul-artistry-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.19"
FRAGMENTS = (
    "src/sheet/209.9296-feat-adv-soul-artistry.js",
    "src/css/59.95-adv-soul-artistry.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("soul-artistry-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "96dda731ef0cf9670fe2c65763bf28e2a73698972e7d8f49014ac598c728385a"
PRE_RELEASE_BYTES = 2780550
PRE_RELEASE_COMMIT = "fe20e753756d8623c5a8f86de63ee341d9595476"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.19(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.19 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END SA4519 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+SA4519\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_SOUL_ARTISTRY_ENABLED|SA4519|sa4519[A-Za-z0-9_-]*|skillFamilyPick)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("soul-artistry-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END SA4519")
