...
(async function runF3Harness(){
  ...
  closeRoll();
  // HARNESS EVOLUTION (Feature 4). These previously pinned the registry to exactly
  // ['range','stance','wounds'] at 20/30/40. That invariant held only until the next feature
  // registered a permanent contributor -- Feature 4 adds 'void@50' -- and would break again at
  // Features 5, 6 and 7. This is the same correction already applied to Feature 2's G12.
  // WHY THE NEW INVARIANT IS CORRECT: what Feature 3 must guarantee is that ITS contributor is
  // registered at ITS priority and that the earlier features' are still present. The full list
  // is not Feature 3's to own, and pinning it made every later feature edit this file.
  ok('F3-G','G3a','Feature 3 registered wounds at priority 40',
     T.PREROLL_MODIFIER_REGISTRY.some(m=>m.id==='wounds' && m.priority===40));
  ok('F3-G','G3b','Earlier contributors still present at their priorities',
     T.PREROLL_MODIFIER_REGISTRY.some(m=>m.id==='range'  && m.priority===20) &&
     T.PREROLL_MODIFIER_REGISTRY.some(m=>m.id==='stance' && m.priority===30));
  eq('F3-G','G4','Registry is ordered by ascending priority',
     T.PREROLL_MODIFIER_REGISTRY.map(m=>m.priority),
     T.PREROLL_MODIFIER_REGISTRY.map(m=>m.priority).slice().sort((a,b)=>a-b));
  eq('F3-G','G4b','No test modifiers leaked',
