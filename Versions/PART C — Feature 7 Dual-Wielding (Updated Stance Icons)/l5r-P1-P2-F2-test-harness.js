...
(async function runF2Harness(){
  ...
  // ICON-SWAP UPDATE. This photographed an implementation detail -- that every stance icon
  // was an inline <svg>. Three of them (attack / defence / full defence) are now the stance
  // artwork, embedded as <img>. The assertion's intent, that all five tiles carry an icon,
  // is unchanged and still enforced; only the element name it accepts has widened.
  eq('F2-A','A5','Every tile has an icon',document.querySelectorAll('.stance-icon svg, .stance-icon img').length,5);
