...
const previous = path.resolve(__dirname, '../../PART K — Phase 12 Play and Management Modes/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Phase 12 \(part 1\) Play and Management Modes/],
  [path.join(__dirname, 'clan-school-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
  const run = spawnSync(process.execPath, [file, sheet], {env:process.env, encoding:'utf8', timeout:3600000, maxBuffer:64 * 1024 * 1024});
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 12.1 (part 2) Clan and School`);
