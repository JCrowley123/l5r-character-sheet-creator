#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12.1 Play Mode Clan and School (part 2) from a SCRATCH COPY of the Phase 0 tree.
...
manifest lines, and the modes-clan-school-seam block in 210-test-seam-and-init.js are removed. The
Clan & School tab goes back to editing in both modes. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12.1"
FRAGMENTS = (
    "src/sheet/209.9997-feat-modes-clan-school.js",
    "src/css/59.998-feat-modes-clan-school.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-clan-school-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "6c860a52b8e3215c7011484e90f8d33c991b347c4211e83fcb9d94a63bf857a9"
PRE_RELEASE_BYTES = 3083017
PRE_RELEASE_COMMIT = "2c287da"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12\.1(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12\.1 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES121 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES121\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES121_ENABLED|MODES121)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-clan-school-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES121")
