/* ============================================================================
   ...
   For: l5r-character-sheet part C feature 8 mirumoto SWIPE.html
...
(async function(){
  ...
  R['track holds every page plus two clones'] = track.children.length === n + 2;
  R['exactly two clones']                     = track.querySelectorAll(':scope > [data-clone]').length === 2;
  R['snap geometry intact']                   = Math.abs(track.scrollWidth - track.children.length * W) < 2;
  ...
  if(slots){
    ...
    await wait(350);
    R['section returns cleanly'] = API.getTabCount() === before;
  }
