#!/usr/bin/env python3
"""Remove Part I Feature 4.57 (Unlucky) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha 5a47ab73...
...
Same safety model as Feature 4.55's and 4.56's removers, and for the same reason: explicit
...
PRE_RELEASE_SHA = "5a47ab73eb955fa03edddf9d16c90a4d9a0c6d90c1e679d0665320f963225ca6"
PRE_RELEASE_BYTES = 2546662
# The Feature 4.56 build this release was added to. Removing 4.57 restores exactly that.
...
END_RE = re.compile(r"\bEND UNLUCKY457\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END UNLUCKY457")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END UNLUCKY457 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
