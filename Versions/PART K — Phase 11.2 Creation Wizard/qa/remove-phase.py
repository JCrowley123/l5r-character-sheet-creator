#!/usr/bin/env python3
"""Surgically remove PART K Phase 11.2 (Creation Wizard) from a SCRATCH COPY of the Phase 0 tree.
...
lines, and the wizard-seam block in 210-test-seam-and-init.js are removed. Phase 11 (Characters
List) depends nothing on this phase and stays. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 11.2"
FRAGMENTS = (
    "src/sheet/209.994-feat-creation-wizard.js",
    "src/css/59.994-feat-creation-wizard.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("wizard-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "6043dabbde6d26f488124d1512be3866906039454cb850fafe1fc3ed53b76a09"
PRE_RELEASE_BYTES = 2958319
PRE_RELEASE_COMMIT = "2b3b8c4"
...
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+11\.2(?![.\d])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 11\.2 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END WIZARD112 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+WIZARD112\b", re.I)
SURFACE_RE = re.compile(r"\b(?:CREATION_WIZARD_ENABLED|CW112|cw112[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("wizard-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END WIZARD112")
