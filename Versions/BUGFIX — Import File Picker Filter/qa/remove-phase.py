#!/usr/bin/env python3
"""Surgically remove BUGFIX — Import File Picker Filter from a SCRATCH COPY of the Phase 0 tree.
...
the import-filter-seam block in 210-test-seam-and-init.js are removed. Both Import controls go
back to offering JSON files only. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "BUGFIX"  # MARKER_RE captures the bare word; the long form is BUGFIX IMPORTFILTER
FRAGMENTS = (
    "src/sheet/209.999-bugfix-import-file-filter.js",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("import-filter-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "480e1a157c4f9a1ad259e4d141d2e262e574da3926909a7d7393f878b6fae890"
PRE_RELEASE_BYTES = 3060080
PRE_RELEASE_COMMIT = "d07b03f"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bBUGFIX\s+IMPORTFILTER\b", re.I)
BEGIN_RE = re.compile(r"^\s*// BUGFIX IMPORTFILTER BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END IMPORTFILTER ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+IMPORTFILTER\b", re.I)
SURFACE_RE = re.compile(r"\b(?:IMPORT_FILE_FILTER_FIX_ENABLED|IMPORT_FILE_FILTER|importFilterPreviousBuild)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("import-filter-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END IMPORTFILTER")
