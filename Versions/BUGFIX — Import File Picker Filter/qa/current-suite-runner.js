...
const previous = path.resolve(__dirname, '../../PART K — Phase 11.2.4 Wizard Starting Spells for Every School/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Phase 11.2.4 Wizard Starting Spells for Every School/],
  [path.join(__dirname, 'import-filter-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus the Import File Picker Filter bugfix`);
