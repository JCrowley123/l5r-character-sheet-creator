#!/usr/bin/env python3
"""Surgically remove BUGFIX — Spell Slot Accounting from a SCRATCH COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY [--dry-run] [--expect-sha SHA256]

Preflights the complete operation before writing. Only the two owned fragments,
their manifest lines, the spell-slot-seam block and the bonus-pip-tap block are removed.
Later features are preserved; the resulting manifest hash is calculated from the
remaining fragments rather than forcing an old baseline. Rebuild the copy with
its build/recombine.py afterwards. No snapshot restore or live-tree edit exists.
"""
...
MARKER = "BUGFIX"  # MARKER_RE captures the bare word; the long form is BUGFIX SPELLSLOT
FRAGMENTS = (
    "src/sheet/209.992-bugfix-spell-slot-accounting.js",
    "src/css/59.992-bugfix-spell-slot-accounting.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("spell-slot-seam",),
                 "src/sheet/080-identity-build-ui.js": ("bonus-pip-tap",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "3e262b188a453aed40451a7162a38e90761f6a04b35ea54bf124b9f1309151a2"
PRE_RELEASE_BYTES = 2914055
PRE_RELEASE_COMMIT = "c814568444586c493798e81a1a1d44538ac795ef"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bBUGFIX\s+SPELLSLOT\b", re.I)
BEGIN_RE = re.compile(r"^\s*// BUGFIX SPELLSLOT BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END SPELLSLOT ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+SPELLSLOT\b", re.I)
SURFACE_RE = re.compile(r"\b(?:SPELL_SLOT_ACCOUNTING_FIX_ENABLED|SPELL_SLOT_REFUND_ENABLED|BONUS_PIP_GUARD_ENABLED|SSA992|ssa992[A-Za-z0-9_-]*|ssaBonusPipTap)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("spell-slot-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END SPELLSLOT")
