...
SPEC = importlib.util.spec_from_file_location("spell_slot_accounting_removal", Path(__file__).with_name("remove-phase.py"))
R = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(R)
SLUG = "spell-slot-seam"
SHARED = "src/sheet/210-test-seam-and-init.js"
PIP_SLUG = "bonus-pip-tap"
PIP_SHARED = "src/sheet/080-identity-build-ui.js"


def block(body="  owned();\n", slug=SLUG, eol="\n"):
    return (f"  before();{eol}  // BUGFIX SPELLSLOT BEGIN {slug}{eol}"
            f"{body}  // END SPELLSLOT {slug}{eol}  after();{eol}")
...
def manifest(eol="\n"):
    return eol.join([
        ...
        f'    {{ "file": "{PIP_SHARED}", "note": "shared handler" }},',
...
def fixture(root, eol="\n"):
    ...
    write(root, R.FRAGMENTS[0], f"// {R.MARKER} spell slot accounting{eol}const SSA992 = {{}};{eol}")
    write(root, R.FRAGMENTS[1], f"/* {R.MARKER} styles */{eol}.ssa992-note {{ color: red; }}{eol}")
    write(root, PIP_SHARED, block(eol=eol, body="  owned();" + eol, slug=PIP_SLUG))
...
class BlockFixtures(unittest.TestCase):
    ...
    def test_mismatched_slug_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "mismatched"):
            R.strip_owned_blocks(block().replace("END SPELLSLOT " + SLUG, "END SPELLSLOT wrong"), "fixture")

    def test_orphan_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "orphan"):
            R.strip_owned_blocks("// END SPELLSLOT spell-slot-seam\n", "fixture")

    def test_missing_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "unclosed"):
            R.strip_owned_blocks(block().replace("  // END SPELLSLOT " + SLUG + "\n", ""), "fixture")

    def test_partial_marker_refused(self):
        for source in [block().replace("BEGIN", "BEG"), block().replace("END SPELLSLOT", "END W451"),
                       block().replace("// BUGFIX", "/* BUGFIX"), block().replace(SLUG, SLUG + " extra")]:
    ...
    def test_partial_end_inside_block_refused(self):
        for partial in ["// END SPELLSLOT", "// end spellslot wrong", "/* END SPELLSLOT wrong */"]:
    ...
    def test_foreign_phase_inside_refused(self):
        for marker in ["PART I FEATURE 4.5.23", "PART J PHASE 8", "Part G Phase 3's preview", "BUGFIX"]:
    ...
    def test_later_phases_outside_block_preserved(self):
        later = "// PART I FEATURE 4.5.240 later phase\nkeepLater();\n// PART I FEATURE 4.5.1 earlier\n// BUGFIX other fix\n"
...
class ScratchFixtures(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-ssa-removal-fixture-")
    ...
    def test_success_only_changes_declared_files(self):
        ...
        self.assertEqual(changed, {SHARED, PIP_SHARED, "build/manifest.json"})
        self.assertEqual(after[SHARED], b"  before();\n  after();\n")
        self.assertEqual(after[PIP_SHARED], b"  before();\n  after();\n")
    ...
    def test_missing_handler_block_refuses_before_writes(self):
        write(self.root, PIP_SHARED, "nothing to remove\n")
        self.refuse_unchanged()

    def test_block_in_the_wrong_file_refuses_before_writes(self):
        write(self.root, PIP_SHARED, block())
        self.refuse_unchanged()

    def test_foreign_marker_in_owned_file_refuses_before_writes(self):
        # BUGFIX is this fix's own marker word, so only other phases' markers are foreign here.
        for marker in ["PART J PHASE 8", "PART I FEATURE 4.5.24"]:
    ...
    def test_unmanifested_leak_refuses_before_writes(self):
        for token in ["SSA992", "SPELL_SLOT_ACCOUNTING_FIX_ENABLED", "BONUS_PIP_GUARD_ENABLED", "ssa992-note", "ssa992BonusNote", "ssa992PreviousCast", "ssaBonusPipTap"]:
...
class ReadOnlyGuardFixtures(unittest.TestCase):
    ...
    def test_missing_live_tree_refuses_blind_operation(self):
        with tempfile.TemporaryDirectory(prefix="l5r-ssa-guard-") as directory:
