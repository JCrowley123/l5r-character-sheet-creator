...
const previous = path.resolve(__dirname, '../../PART I — Phase 4.5.24 Touch of the Spirit Realms/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Touch of the Spirit Realms/],
  [path.join(__dirname, 'spell-slot-accounting-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Spell Slot Accounting`);
