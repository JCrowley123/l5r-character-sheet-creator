#!/usr/bin/env python3
"""Surgically remove the per-roll declaration registry from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, the four preview-* blocks in Phase 3's (Part G) preview
and the roll-declarations-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.15"
FRAGMENTS = (
    "src/sheet/209.927-feat-roll-declarations.js",
    "src/css/59.91-roll-declarations.css",
)
SHARED_BLOCKS = {
    "src/sheet/208-feat-roll-preview.js": ("preview-start", "preview-cancel", "preview-html", "preview-toggle"),
    "src/sheet/210-test-seam-and-init.js": ("roll-declarations-seam",),
}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "61de1d40772cb00abba41f7ab5d2d151027b3554545d04e4f8bf127d39adaa48"
PRE_RELEASE_BYTES = 2730118
PRE_RELEASE_COMMIT = "0055ec53fa71b22ab0e71127bb869fd2c092ef6f"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.15(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.15 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END DECL4515 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+DECL4515\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ROLL_DECLARATIONS_ENABLED|RD4515|DECL4515|rd4515[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("roll-declarations-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END DECL4515")
