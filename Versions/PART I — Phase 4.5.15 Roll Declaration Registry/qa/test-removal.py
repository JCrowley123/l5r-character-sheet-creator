...
SPEC = importlib.util.spec_from_file_location("roll_declarations_removal", Path(__file__).with_name("remove-phase.py"))
R = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(R)
SLUG = "roll-declarations-seam"
SHARED = "src/sheet/210-test-seam-and-init.js"
PREVIEW = "src/sheet/208-feat-roll-preview.js"
PREVIEW_SLUGS = R.SHARED_BLOCKS[PREVIEW]


def block(body="  owned();\n", slug=SLUG, eol="\n"):
    return (f"  before();{eol}  // {R.MARKER} BEGIN {slug}{eol}"
            f"{body}  // END DECL4515 {slug}{eol}  after();{eol}")
...
def manifest(eol="\n"):
    return eol.join([
        ...
        f'    {{ "file": "{PREVIEW}", "note": "preview" }},',
        f'    {{ "file": "{SHARED}", "note": "shared" }}', '  ]', '}', ''])


def preview(eol="\n"):
    return "".join(block(slug=s, eol=eol, body="  owned();" + eol) for s in PREVIEW_SLUGS)
...
def fixture(root, eol="\n"):
    ...
    write(root, R.FRAGMENTS[0], f"// {R.MARKER} declarations{eol}const RD4515 = {{}};{eol}")
    write(root, R.FRAGMENTS[1], f"/* {R.MARKER} styles */{eol}.rd4515-opt {{ color: red; }}{eol}")
    write(root, PREVIEW, preview(eol))
...
class BlockFixtures(unittest.TestCase):
    ...
    def test_mismatched_slug_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "mismatched"):
            R.strip_owned_blocks(block().replace("END DECL4515 " + SLUG, "END DECL4515 wrong"), "fixture")

    def test_orphan_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "orphan"):
            R.strip_owned_blocks("// END DECL4515 roll-declarations-seam\n", "fixture")

    def test_missing_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "unclosed"):
            R.strip_owned_blocks(block().replace("  // END DECL4515 " + SLUG + "\n", ""), "fixture")

    def test_partial_marker_refused(self):
        for source in [block().replace("BEGIN", "BEG"), block().replace("END DECL4515", "END DECL451"),
    ...
    def test_partial_end_inside_block_refused(self):
        for partial in ["// END DECL4515", "// end decl4515 wrong", "/* END DECL4515 wrong */"]:
    ...
    def test_foreign_phase_inside_refused(self):
        for marker in ["PART I FEATURE 4.5.14", "PART J PHASE 8", "Part G Phase 3's preview", "BUGFIX"]:
    ...
    def test_later_phases_outside_block_preserved(self):
        later = "// PART I FEATURE 4.5.150 later phase\nkeepLater();\n// PART I FEATURE 4.5.1 earlier\n// BUGFIX other fix\n"
...
class ScratchFixtures(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-decl-removal-fixture-")
    ...
    def test_success_only_changes_declared_files(self):
        ...
        self.assertEqual(changed, {SHARED, PREVIEW, "build/manifest.json"})
        self.assertEqual(after[SHARED], b"  before();\n  after();\n")
        self.assertEqual(after[PREVIEW], b"  before();\n  after();\n" * len(PREVIEW_SLUGS))
    ...
    def test_missing_one_preview_block_refuses_before_writes(self):
        for slug in PREVIEW_SLUGS:
            with self.subTest(slug=slug):
                fixture(self.root)
                write(self.root, PREVIEW, "".join(block(slug=s) for s in PREVIEW_SLUGS if s != slug))
                self.refuse_unchanged()

    def test_foreign_marker_in_preview_block_refuses_before_writes(self):
        write(self.root, PREVIEW, preview().replace("  owned();", "  // PART I FEATURE 4.5.11 prose", 1))
        self.refuse_unchanged()
    ...
    def test_unmanifested_leak_refuses_before_writes(self):
        for token in ["RD4515", "ROLL_DECLARATIONS_ENABLED", "rd4515-opt", "rd4515PreviewHtml", "data-rd4515-key", "DECL4515"]:
...
class ReadOnlyGuardFixtures(unittest.TestCase):
    ...
    def test_missing_live_tree_refuses_blind_operation(self):
        with tempfile.TemporaryDirectory(prefix="l5r-decl-guard-") as directory:
