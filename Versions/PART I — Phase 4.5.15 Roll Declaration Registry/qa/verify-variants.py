...
BASE = '0055ec53fa71b22ab0e71127bb869fd2c092ef6f'
BASE_SHA = '61de1d40772cb00abba41f7ab5d2d151027b3554545d04e4f8bf127d39adaa48'
RETAINED = (1353, 1353)
BASE_PREV = '9dd3fef0c86e4494ef5ceb75a47e502ec78d0639'
JS = 'src/sheet/209.927-feat-roll-declarations.js'
CSS = 'src/css/59.91-roll-declarations.css'
MUTATIONS = {
    'no-damage-exclusion': (b'      context.kind !== ROLL_KINDS.DAMAGE;\n', b'      true;\n'),
    'no-context-binding': (b'      if(!state || state.context !== context) return [];\n      const offered',
                           b'      if(!state) return [];\n      const offered'),
    'cancel-keeps-armed': (b'api.cancel = function(){ state = null; };', b'api.cancel = function(){};'),
    'start-keeps-armed': (b'{context:context, armed:{}}', b'{context:context, armed:state ? state.armed : {}}'),
    'no-dice-route': (b'.concat(RD4515.modifiers(context));', b'.concat([]);'),
    'label-not-escaped': (b"'><span>' + escHtml(o.label) + '</span></label>'", b"'><span>' + o.label + '</span></label>'"),
    'provider-errors-not-isolated': (b'try { list = p.offers(context); } catch(e){ list = []; }', b'list = p.offers(context);'),
    'deltas-not-normalised': (b'rolledDelta:num(m.rolledDelta)', b'rolledDelta:m.rolledDelta'),
}
...
def removal_order(scratch, opts, report, failures):
    """4.5.14 removed while this release stays; then both removed, in each order."""
    remover_prev = REPO / 'Versions/PART I — Phase 4.5.14 Darling of the Court and Servant/qa/remove-phase.py'
    runner_prev = REPO / 'Versions/PART I — Phase 4.5.13 Named Advantages/qa/current-suite-runner.js'
    ...
    if any(step.returncode for step in steps):
        failures.append('4.5.14-first removal failed: ' + ''.join(step.stdout + step.stderr for step in steps)[-400:])
        return
    output = tree / 'l5r-character-sheet.html'
    for label, harness, expected in [('this-release-harness', HERE / 'roll-declarations-harness.js', None),
                                     ('pre-4514-retained', runner_prev, (1128, 1128))]:
        run = execute([opts.node, harness, output])
        counts = counts_of(run)
        order['4514-removed-' + label] = {'passed': counts[0], 'total': counts[1], 'exit': run.returncode}
        if run.returncode or counts[1] == 0 or counts[0] != counts[1] or (expected and counts != expected):
            failures.append('with 4.5.14 removed, ' + label + ' failed')
    for label, first, second in [('4515-then-4514', HERE / 'remove-phase.py', remover_prev),
                                 ('4514-then-4515', remover_prev, HERE / 'remove-phase.py')]:
        tree = scratch / label
        shutil.copytree(PHASE, tree, ignore=shutil.ignore_patterns('__pycache__'))
        ...
        order[label] = {'byte_identical_to_4514_restore_point': identical, 'bytes': len(output),
                        'sha256': hashlib.sha256(output).hexdigest()}
        if not identical:
            failures.append(label + ' removal did not reach the 4.5.14 restore point')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='l5r-decl-qa-') as directory:
        ...
        for name in ['disabled', 'no-styles', *MUTATIONS, 'removed']:
            ...
            if name == 'disabled':
                ...
                old = b'const ROLL_DECLARATIONS_ENABLED = true;'
                assert data.count(old) == 1, 'kill-switch mutation anchor must be unique'
                (tree / JS).write_bytes(data.replace(old, b'const ROLL_DECLARATIONS_ENABLED = false;'))
            elif name == 'no-styles':
            ...
            if name == 'removed':
                ...
                if not opts.retained:
                    continue
                harness = REPO / 'Versions/PART I — Phase 4.5.14 Darling of the Court and Servant/qa/current-suite-runner.js'
            else:
                harness = HERE / 'roll-declarations-harness.js'
            run = execute([opts.node, harness, output])
            matches = re.findall(r'(?:COMBINED )?(\d+)/(\d+) checks passed', run.stdout)
    ...
    print('RD4515_VARIANT_RESULT=' + json.dumps(report, ensure_ascii=True), flush=True)
