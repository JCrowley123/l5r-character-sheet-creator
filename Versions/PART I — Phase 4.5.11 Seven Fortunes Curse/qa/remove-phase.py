#!/usr/bin/env python3
"""Remove Part I Feature 4.5.11 (Seven Fortunes' Curse, D04a) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha 912ab336...
...
TWO shared files rather than one, the same pair Feature 4.5.10 touched: the seam, and Phase 3's
208-feat-roll-preview.js, where Ebisu's and Jurojin's per-roll declarations need four guarded
hooks. That preview still has no generic registration point -- see this phase's README for why
generalising it was deliberately left to its own refactor phase rather than done here. All four
blocks are ordinary delimited blocks and come out exactly the way the seam block does.

NOT touched here, deliberately: this release also corrected FIVE checks in four other phases'
harnesses (4.5.2's, 4.5.3's, 4.5.8's, 4.5.9's and 4.5.10's). Those live under Versions/, not in
the Phase 0 tree, and every one is written to read correctly whether or not this phase is present
-- which is the whole reason those shapes were chosen. Removing this phase needs no edit to them.
See ROLLBACK.md, "Cross-phase fixture corrections".

Everything is validated before a byte is written, and the live Phase 0 tree is refused outright.

Same safety model as Features 4.55 through 4.5.10's removers, and for the same reason: explicit
delimiters rather than "this marker owns everything until the next one", which is the weaker rule
that let one phase silently take ownership of four of another's CSS rules (CLAUDE.md,
"Re-verify removability at the END of a phase").
"""
...
FRAGMENTS = (
    "src/sheet/209.97-feat-disadv-fortune.js",
    "src/css/59.6-disadv-fortune.css",
)
SHARED_FILES = (
    "src/sheet/210-test-seam-and-init.js",
    "src/sheet/208-feat-roll-preview.js",
)
PRE_RELEASE_SHA = "812ac85e88be88e261a834330fd976612a467448f5ad63da16c096ee052db13a"
PRE_RELEASE_BYTES = 2617077
# The Feature 4.5.10 build this release was added to, INCLUDING that phase's two same-day
# real-device corrections. Removing 4.5.11 restores exactly that.
...
END_RE = re.compile(r"\bEND FORTUNE4511\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END FORTUNE4511")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END FORTUNE4511 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
