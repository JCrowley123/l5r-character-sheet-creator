...
(async function runF4Harness(){
  ...
  // FEATURE 7 SNAPSHOT UPDATE (second one; Feature 6 made the first). This assertion
  // photographs the WHOLE registry, so it changes by construction every time a later feature
  // registers a contributor. Feature 7 adds offhand@35, between stance (30) and wounds (40).
  // Every other Feature 4 assertion is untouched, and the off-hand contributor is silent on
  // every roll Feature 4 makes (it returns null unless the roll declared a hand).
  eq('F4-A','A3','Registry is range/arrow/stance/offhand/wounds/void at 20/25/30/35/40/50',
