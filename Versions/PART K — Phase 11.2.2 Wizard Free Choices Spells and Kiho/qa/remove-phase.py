#!/usr/bin/env python3
"""Surgically remove PART K Phase 11.2.2 (Wizard Free Choices, Spells and Kiho) from a SCRATCH COPY of the
Phase 0 tree.
...
lines, and the wizard3-seam block in 210-test-seam-and-init.js are removed. Phases 11.2 and
11.2.1 stay, back to their eight steps. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 11.2.2"
FRAGMENTS = (
    "src/sheet/209.996-feat-wizard-free-choices.js",
    "src/css/59.996-feat-wizard-free-choices.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("wizard3-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "ee04939670db1817823f709974ef13207282d2a0f7df87c26e3c760883eb2939"
PRE_RELEASE_BYTES = 3003661
PRE_RELEASE_COMMIT = "7f25263"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
# (?![.\d]) so a later 11.2.2.x marker is never mistaken for this one; the earlier 11.2 and 11.2.1
# markers cannot match either, because this pattern needs the ".2.2".
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+11\.2\.2(?![.\d])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 11\.2\.2 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END WIZARD1122 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+WIZARD1122\b", re.I)
SURFACE_RE = re.compile(r"\b(?:WIZARD_FREE_CHOICES_ENABLED|CW1122|cw1122[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("wizard3-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END WIZARD1122")
