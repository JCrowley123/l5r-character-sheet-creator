...
| 5 · Elemental books | yes | `kihoEligibility()` (a `monksOnly` check for Rebuke of the Heavens, new `monkOnlyBlock` in its return), the dropdown lock-reason list, `cfs_applySchool` and `renderSchoolInfo()` and the Apply status message (all three now tolerate `honor:null`), `DISADV_LIBRARY` (Uncentered), `KIHO_BAN_BY_DISADVANTAGE` (the row Phase 1 left empty), and the six existing Brotherhood entries now share the new `MONK_OUTFIT` constant instead of repeating the string. The 29 Kiho and 11 Schools themselves are pure data. |
| 6 · Alternate Paths | yes | `unlockTechniques()` (one branch substituting a Path's Technique at its Rank — `f_schoolTechGranted` keeps its existing shape, deliberately), `kihoExclusions()` and `kihoEntitlement()` (both now read the active Paths as a second source), `cfs_applySchool` (clears the Path record), `recalcAll()` (one added call), three Dragon `SCHOOL_LIBRARY` entries gain `tags:['tattooed order']`, and `TECH_DESCRIPTIONS` (twelve new keys). Adds `f_pathTaken` (44th f_ field) plus the `pathPicker`, `pathPickerWrap` and `pathNote` elements. `ALTERNATE_PATH_LIBRARY`, the resolver and the matcher are additive. |
| 7 · later books | yes | `cfs_applySchool` (a new player-choice gate for starting Honor, and `honorToSet` replacing the direct `sc.honor` write), the Apply status message, `renderSchoolInfo()` (Honor choices and the setting note), `kihoEntitlement()` (`capExemptGrants` now reads the active School too), and `pickAffinityElement()` — which now sets its own modal heading, because `pickStartingHonor()` shares that modal. The five Schools and their Technique text are pure data. No new element ids. |
