#!/usr/bin/env python3
"""Surgically remove the Wealthy koku grant from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact wealthy-grant-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.17"
FRAGMENTS = (
    "src/sheet/209.929-feat-adv-wealthy-grant.js",
    "src/css/59.93-adv-wealthy-grant.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("wealthy-grant-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "0aefe9c90c964eb50132bd0782b433648fee31ca069d7fc3a5c35615ea98163a"
PRE_RELEASE_BYTES = 2753162
PRE_RELEASE_COMMIT = "65d21069fd6ad937a67f74023fbbe308452fb706"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.17(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.17 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END W4517 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+W4517\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_WEALTHY_GRANT_ENABLED|W4517|wealth4517[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("wealthy-grant-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END W4517")
