...
BASE = '65d21069fd6ad937a67f74023fbbe308452fb706'
BASE_SHA = '0aefe9c90c964eb50132bd0782b433648fee31ca069d7fc3a5c35615ea98163a'
RETAINED = (1497, 1497)
BASE_PREV = '45d0f489081de3c5740c60b74afb0714396b6a6d'
JS = 'src/sheet/209.929-feat-adv-wealthy-grant.js'
CSS = 'src/css/59.93-adv-wealthy-grant.css'
MUTATIONS = {
    'no-minimum': (b"return price.cost < 1 ? Object.assign({}, price, {cost:1, floored:true}) : price;", b"return price;"),
    'mint-on-render': (b"      const s = api.status(div);\n      const row = div && div.querySelector('.adv-config-row');\n",
                       b"      const s = api.status(div);\n      if(s && s.kind === 'unrecorded') { api.act(div, 'add'); return; }\n      const row = div && div.querySelector('.adv-config-row');\n"),
    'raise-adds-whole-target': (b"api.setKoku(api.koku() + target - granted);", b"api.setKoku(api.koku() + target);"),
    'lower-claws-back': (b"if(target > granted){", b"if(target !== granted){"),
    'legacy-defaults-add': (b"(fresh ? ' checked' : '')", b"' checked'"),
    'invalid-receipt-trusted': (b"r.revision === 1 &&", b"true &&"),
    'choice-not-required': (b"        if(!choice){\n", b"        if(false){\n"),
    'return-enabled-when-short': (b"if(disabled) button.disabled = true;", b""),
}
...
def removal_order(scratch, opts, report, failures):
    """4.5.16 removed while this release stays; then both removed, in each order."""
    remover_prev = REPO / 'Versions/PART I — Phase 4.5.16 Heart of Vengeance/qa/remove-phase.py'
    runner_prev = REPO / 'Versions/PART I — Phase 4.5.15 Roll Declaration Registry/qa/current-suite-runner.js'
    ...
    if any(step.returncode for step in steps):
        failures.append('4.5.16-first removal failed: ' + ''.join(step.stdout + step.stderr for step in steps)[-400:])
        return
    output = tree / 'l5r-character-sheet.html'
    for label, harness, expected in [('this-release-harness', HERE / 'wealthy-grant-harness.js', None),
                                     ('pre-4516-retained', runner_prev, (1406, 1406))]:
        run = execute([opts.node, harness, output])
        counts = counts_of(run)
        order['4516-removed-' + label] = {'passed': counts[0], 'total': counts[1], 'exit': run.returncode}
        if run.returncode or counts[1] == 0 or counts[0] != counts[1] or (expected and counts != expected):
            failures.append('with 4.5.16 removed, ' + label + ' failed')
    for label, first, second in [('4517-then-4516', HERE / 'remove-phase.py', remover_prev),
                                 ('4516-then-4517', remover_prev, HERE / 'remove-phase.py')]:
        tree = scratch / label
        shutil.copytree(PHASE, tree, ignore=shutil.ignore_patterns('__pycache__'))
        ...
        order[label] = {'byte_identical_to_4515_restore_point': identical, 'bytes': len(output),
                        'sha256': hashlib.sha256(output).hexdigest()}
        if not identical:
            failures.append(label + ' removal did not reach the 4.5.15 restore point')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='l5r-w-qa-') as directory:
        ...
        for name in ['disabled', 'no-styles', *MUTATIONS, 'removed']:
            ...
            if name == 'disabled':
                ...
                old = b'const ADV_WEALTHY_GRANT_ENABLED = true;'
                assert data.count(old) == 1, 'kill-switch mutation anchor must be unique'
                (tree / JS).write_bytes(data.replace(old, b'const ADV_WEALTHY_GRANT_ENABLED = false;'))
            elif name == 'no-styles':
            ...
            if name == 'removed':
                ...
                if not opts.retained:
                    continue
                harness = REPO / 'Versions/PART I — Phase 4.5.16 Heart of Vengeance/qa/current-suite-runner.js'
            else:
                harness = HERE / 'wealthy-grant-harness.js'
            run = execute([opts.node, harness, output])
            matches = re.findall(r'(?:COMBINED )?(\d+)/(\d+) checks passed', run.stdout)
    ...
    print('W4517_VARIANT_RESULT=' + json.dumps(report, ensure_ascii=True), flush=True)
