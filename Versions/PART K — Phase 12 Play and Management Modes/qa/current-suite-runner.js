...
const previous = path.resolve(__dirname, '../../BUGFIX — Apply School Skill Rows/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus the Apply School Skill Rows bugfix/],
  [path.join(__dirname, 'modes-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 12 (part 1) Play and Management Modes`);
