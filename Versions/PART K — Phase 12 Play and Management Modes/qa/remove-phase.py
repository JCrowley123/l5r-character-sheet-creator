#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12 Play and Management Modes (part 1) from a SCRATCH COPY of the Phase 0 tree.
...
manifest lines, and the modes-seam block in 210-test-seam-and-init.js are removed. The sheet goes
back to having one mode. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12"
FRAGMENTS = (
    "src/sheet/209.9996-feat-play-management-modes.js",
    "src/css/59.997-feat-play-management-modes.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "0dcb56e8c02efbb1c7dba6e9d61da29b7e355009f47b2777c7d481bbdb1ed5d1"
PRE_RELEASE_BYTES = 3072896
PRE_RELEASE_COMMIT = "e66c4dd"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES12 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES12\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES12_ENABLED|MODES12|modes12[A-Za-z]*|pm12Toggle|pm12-[a-z-]+)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES12")
