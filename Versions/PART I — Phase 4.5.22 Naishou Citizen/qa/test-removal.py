...
SPEC = importlib.util.spec_from_file_location("naishou_citizen_removal", Path(__file__).with_name("remove-phase.py"))
R = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(R)
SLUG = "naishou-citizen-seam"
SHARED = "src/sheet/210-test-seam-and-init.js"


def block(body="  owned();\n", slug=SLUG, eol="\n"):
    return (f"  before();{eol}  // {R.MARKER} BEGIN {slug}{eol}"
            f"{body}  // END NC4522 {slug}{eol}  after();{eol}")
...
def fixture(root, eol="\n"):
    ...
    write(root, R.FRAGMENTS[0], f"// {R.MARKER} naishou citizen{eol}const NC4522 = {{}};{eol}")
    write(root, R.FRAGMENTS[1], f"/* {R.MARKER} styles */{eol}.nc4522-badge {{ color: red; }}{eol}")
...
class BlockFixtures(unittest.TestCase):
    ...
    def test_mismatched_slug_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "mismatched"):
            R.strip_owned_blocks(block().replace("END NC4522 " + SLUG, "END NC4522 wrong"), "fixture")

    def test_orphan_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "orphan"):
            R.strip_owned_blocks("// END NC4522 naishou-citizen-seam\n", "fixture")

    def test_missing_end_refused(self):
        with self.assertRaisesRegex(R.RemovalError, "unclosed"):
            R.strip_owned_blocks(block().replace("  // END NC4522 " + SLUG + "\n", ""), "fixture")

    def test_partial_marker_refused(self):
        for source in [block().replace("BEGIN", "BEG"), block().replace("END NC4522", "END W451"),
    ...
    def test_partial_end_inside_block_refused(self):
        for partial in ["// END NC4522", "// end nc4522 wrong", "/* END NC4522 wrong */"]:
    ...
    def test_foreign_phase_inside_refused(self):
        for marker in ["PART I FEATURE 4.5.21", "PART J PHASE 8", "Part G Phase 3's preview", "BUGFIX"]:
    ...
    def test_later_phases_outside_block_preserved(self):
        later = "// PART I FEATURE 4.5.220 later phase\nkeepLater();\n// PART I FEATURE 4.5.1 earlier\n// BUGFIX other fix\n"
...
class ScratchFixtures(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="l5r-nc-removal-fixture-")
    ...
    def test_unmanifested_leak_refuses_before_writes(self):
        for token in ["NC4522", "ADV_NAISHOU_CITIZEN_ENABLED", "nc4522-badge", "nc4522-blessing-note", "nc4522Previous", "nc4522"]:
...
class ReadOnlyGuardFixtures(unittest.TestCase):
    ...
    def test_missing_live_tree_refuses_blind_operation(self):
        with tempfile.TemporaryDirectory(prefix="l5r-nc-guard-") as directory:
