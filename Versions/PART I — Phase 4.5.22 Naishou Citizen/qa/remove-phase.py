#!/usr/bin/env python3
"""Surgically remove Naishou Citizen from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact naishou-citizen-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.22"
FRAGMENTS = (
    "src/sheet/209.9299-feat-adv-naishou-citizen.js",
    "src/css/59.98-adv-naishou-citizen.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("naishou-citizen-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "9c749b7dc9c45fc5829f8db3172067d91ced54833e734da4f245d3ddb08528d0"
PRE_RELEASE_BYTES = 2846031
PRE_RELEASE_COMMIT = "436372ad4e0782417c4efcb7dd20f3a342b3c7ee"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.22(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.22 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END NC4522 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+NC4522\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_NAISHOU_CITIZEN_ENABLED|NC4522|nc4522[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("naishou-citizen-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END NC4522")
