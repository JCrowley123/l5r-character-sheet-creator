...
const previous = path.resolve(__dirname, '../../BUGFIX — Spell Slot Accounting/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Spell Slot Accounting/],
  [path.join(__dirname, 'safari-tab-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Spell Slots Tab on Safari`);
