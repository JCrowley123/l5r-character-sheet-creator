#!/usr/bin/env python3
"""Surgically remove BUGFIX — Spell Slots Tab on Safari from a SCRATCH COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY [--dry-run] [--expect-sha SHA256]

Preflights the complete operation before writing. This fix owns no fragment: only the
probe-switch and hidden-page-probe blocks in src/layer/10-carousel.js are removed, and
expect_sha256 is recomputed.
...
MARKER = "BUGFIX"  # MARKER_RE captures the bare word; the long form is BUGFIX SAFARITAB
FRAGMENTS: tuple[str, ...] = ()
SHARED_BLOCKS = {"src/layer/10-carousel.js": ("probe-switch", "hidden-page-probe")}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "339a9590bb9761441db94abb776afc1df9e260d2647114f6f706d297c011e726"
PRE_RELEASE_BYTES = 2927339
PRE_RELEASE_COMMIT = "8cbfa39"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bBUGFIX\s+SAFARITAB\b", re.I)
BEGIN_RE = re.compile(r"^\s*// BUGFIX SAFARITAB BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END SAFARITAB ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+SAFARITAB\b", re.I)
SURFACE_RE = re.compile(r"\bSAFARI_TAB_PROBE_ENABLED\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("probe-switch", "hidden-page-probe")) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END SAFARITAB")
