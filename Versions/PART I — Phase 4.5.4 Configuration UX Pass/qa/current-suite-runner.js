/* Current regression entry point, as of Feature 4.54.
 *
 * Delegates to Feature 4.53's runner UNCHANGED — which in turn delegates to 4.5.2's — and adds
 * this release's own suite, reporting one combined total. When 4.54 is rolled back this file
 * goes with its folder and 4.53's runner is the entry point again.
 *
...
const previous = path.join(repo, 'Versions/PART I — Phase 4.5.3 Configuration Repairs/qa/current-suite-runner.js');
const own = path.join(__dirname, 'adv-config-ux-harness.js');
...
const previousOutput = runOne('Feature 4.53 suite set', previous, [sheet]);
...
const ownOutput = runOne('Feature 4.54 UX pass', own, [sheet]);
const ownCounts = /(\d+)\/(\d+) checks passed/.exec(ownOutput);
if (ownCounts) { passed += Number(ownCounts[1]); total += Number(ownCounts[2]); }
else { console.log('FAIL the 4.54 harness produced no count'); failures += 1; }

console.log(`\nCOMBINED ${passed}/${total} checks passed across every retained suite plus Feature 4.54`);
