#!/usr/bin/env python3
"""Remove Part I Feature 4.54 (Configuration UX Pass) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha 18a740e8...
...
Same safety model as Feature 4.53's remover, and for the same reason: explicit
...
END_RE = re.compile(r"\bEND UX454\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END UX454")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END UX454 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
