#!/usr/bin/env python3
"""Positive and negative fixtures for Feature 4.55's removal tool.
...
MARKER = "PART I FEATURE 4.55"


class BlockTests(unittest.TestCase):
    def wrap(self, body, slug="eligibility-seam", eol="\n"):
        return (f"  before();{eol}"
                f"  // {MARKER} BEGIN {slug}{eol}"
                f"{body}"
                f"  // END GATES455 {slug}{eol}"
                f"  after();")
    ...
    def test_owned_block_is_cut_and_neighbours_survive_exactly(self):
        ...
        self.assertEqual(removed, ["eligibility-seam"])
    ...
    def test_the_sibling_release_marker_is_left_alone(self):
        # 4.54's ux-seam sits directly ABOVE this release's block in the real seam file, and
        # "PART I FEATURE 4.5" is a prefix of both markers. A remover matching on a prefix rather
        # than on the exact marker would eat 4.54's block on its way past. Two releases deep now,
        # so the fixture carries both neighbours: 4.53's block is above 4.54's.
        source = ("  // PART I FEATURE 4.53 BEGIN repairs-seam\n  older();\n"
                  "  // END REPAIRS453 repairs-seam\n"
                  "  // PART I FEATURE 4.54 BEGIN ux-seam\n  theirs();\n"
                  "  // END UX454 ux-seam\n"
                  f"  // {MARKER} BEGIN eligibility-seam\n  ours();\n"
...
class RefusalTests(unittest.TestCase):
    def test_foreign_marker_inside_our_block_is_refused(self):
        source = (f"  // {MARKER} BEGIN eligibility-seam\n  // PART J PHASE 5\n  theirs();\n"
                  "  // END GATES455 eligibility-seam\n")
    ...
    def test_orphan_end_delimiter_is_refused(self):
        with self.assertRaises(REMOVAL.RemovalError) as caught:
            REMOVAL.strip_owned_blocks("  // END GATES455 eligibility-seam\n", "shared.js")
...
class ManifestTests(unittest.TestCase):
    ...
    def manifest(self, js=True, css=True, sha="deadbeef"):
        entries = ['    { "file": "src/css/57-disadv-nemesis.css",\n',
                   '      "note": "4.522" },\n']
        if css:
            entries += ['    { "file": "src/css/59-adv-eligibility.css",\n',
                        '      "note": "4.55 css" },\n']
        entries += ['    { "file": "src/sheet/209.89-feat-adv-config-repairs.js",\n',
                    '      "note": "4.53" },\n']
        if js:
            entries += ['    { "file": "src/sheet/209.91-feat-adv-eligibility-gates.js",\n',
                        '      "note": "4.55 js" },\n']
    ...
    def test_both_entries_are_cut_and_the_rest_is_byte_identical(self):
        ...
        self.assertNotIn("src/sheet/209.91-feat-adv-eligibility-gates.js", files)
        self.assertNotIn("src/css/59-adv-eligibility.css", files)
...
class RecordedTargetTests(unittest.TestCase):
    def test_pre_release_build_is_the_one_this_release_was_added_to(self):
        self.assertEqual(REMOVAL.PRE_RELEASE_SHA,
                         "da0db0946afa356df26e9ea79cfb82f87a669d479247ec1ca230e9bb17be7b13")
        self.assertEqual(REMOVAL.PRE_RELEASE_BYTES, 2515953)

    def test_both_halves_of_this_release_are_listed_for_deletion(self):
        self.assertEqual(set(REMOVAL.FRAGMENTS), {
            "src/sheet/209.91-feat-adv-eligibility-gates.js",
            "src/css/59-adv-eligibility.css",
        })
