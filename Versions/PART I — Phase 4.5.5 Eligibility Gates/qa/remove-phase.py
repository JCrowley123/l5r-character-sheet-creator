#!/usr/bin/env python3
"""Remove Part I Feature 4.55 (Eligibility Gates) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha da0db094...

Surgical deletion, not a snapshot restore: it deletes this release's own two files,
their manifest entries, and only the block its own BEGIN/END delimiters enclose.
Everything is validated before a byte is written, and the live Phase 0 tree is
refused outright.

Same safety model as Feature 4.53's and 4.54's removers, and for the same reason: explicit
...
PRE_RELEASE_SHA = "da0db0946afa356df26e9ea79cfb82f87a669d479247ec1ca230e9bb17be7b13"
PRE_RELEASE_BYTES = 2515953
# The Feature 4.54 build this release was added to, AFTER both of that phase's same-day
# real-device corrections. Removing 4.55 restores exactly that, not 4.54's pre-release target.
...
END_RE = re.compile(r"\bEND GATES455\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END GATES455")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END GATES455 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
