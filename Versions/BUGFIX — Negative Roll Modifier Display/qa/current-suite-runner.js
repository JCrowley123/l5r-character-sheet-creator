/* Current regression entry point, as of the Negative Roll Modifier Display bugfix.
 *
 * Delegates to the Mastery Rank Labelling runner UNCHANGED — which in turn delegates to 4.5.11's, and so on
...
const previous = path.join(repo,
  'Versions/BUGFIX — Mastery Rank Labelling/qa/current-suite-runner.js');
const own = path.join(__dirname, 'negative-roll-modifier-harness.js');
...
const previousOutput = runOne('Mastery Rank Labelling suite set', previous, [sheet]);
const trailer = /COMBINED (\d+)\/(\d+) checks passed/.exec(previousOutput);
if (trailer) { passed += Number(trailer[1]); total += Number(trailer[2]); }
else { console.log('FAIL the Mastery Rank Labelling runner produced no combined total'); failures += 1; }

const ownOutput = runOne('BUGFIX Negative Roll Modifier Display', own, [sheet]);
const ownCounts = /(\d+)\/(\d+) checks passed/.exec(ownOutput);
if (ownCounts) { passed += Number(ownCounts[1]); total += Number(ownCounts[2]); }
else { console.log('FAIL the negative-roll-modifier harness produced no count'); failures += 1; }
...
console.log(`\nCOMBINED ${passed}/${total} checks passed across every retained suite plus the Negative Roll Modifier Display bugfix`);
