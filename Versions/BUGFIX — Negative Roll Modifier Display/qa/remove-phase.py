#!/usr/bin/env python3
"""Remove the Negative Roll Modifier Display bugfix from a COPY of the tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha 6722adcb...

THIS REMOVER IS NOT THE SAME SHAPE AS THE MASTERY RANK LABELLING ONE, and the difference is the
point. That fix was purely ADDITIVE -- every block it added could simply be cut. This one REWRITES
two existing expressions in trunk code, because a hardcoded "+" cannot be corrected by inserting
something next to it. So each block here is cut and the trunk's ORIGINAL line put back in its
place, from the verbatim text stored in RESTORE below, and the rebuild must still hash to the
pre-fix build exactly.

That is a stronger proof than it sounds: the fix is only removable at all because both blocks keep
the trunk's own expression as their guarded fallback, so the original text is sitting right there
in the live tree to be compared against.

REMOVING THIS FIX RESTORES THE DEFECT, which is what a faithful rollback of a bugfix means and is
stated rather than buried: the roll modal goes back to printing "+ -40 bonus" in the keep-note and
"10k5 +-40" in the notation. No number moves either way -- the arithmetic was never wrong.

TWO shared files: 210-test-seam-and-init.js (the seam) and 100-dice-engine.js, which is TRUNK code
-- Part B's dice engine -- and is where both malformed strings are built.
...
eight Part I removers still carry.
"""
...
FRAGMENTS = (
    "src/sheet/209.991-bugfix-negative-roll-modifier.js",
)
...
PRE_RELEASE_SHA = "6722adcb613d50853b538a8d24533a7a57866a9d926dfa2c026d5894d9fed0e8"
PRE_RELEASE_BYTES = 2682769
# The Mastery Rank Labelling build this fix was applied to. Removing it restores exactly that.

# The trunk's own text, verbatim, for every block this fix REWRITES rather than adds. Keyed by
# slug. A block whose slug is absent here is cut outright; a block listed here is cut and this put
# back in its place. Stored as whole lines including their newline, so restoring is exact.
RESTORE = {
    "notation":
        "      const adjNotation = `${result.numDice}k${result.keepDice}"
...
END_RE = re.compile(r"\bEND NEGMOD\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END NEGMOD")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END NEGMOD {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
            found = marker_of(current)
            # MARKER_RE's alternation captures the bare word "BUGFIX" out of this fix's own BEGIN
            # lines, never the full "BUGFIX NEGMOD", so both spellings count as ours. That is
        ...
        # A rewritten site is restored, not merely deleted. Cutting it would leave the roll modal
        # with no notation and no keep-note at all rather than with the wording it had before.
        if slug in RESTORE:
            output.append(RESTORE[slug])
        removed.append(slug)
...
def main() -> int:
    ...
    for relative in SHARED_FILES:
        ...
        # Every rewritten site must have been seen. A RESTORE key that no block claimed means the
        # live tree has drifted from what this remover was written against, and a rebuild that
        # happened to hash correctly would be luck rather than proof.
        for slug in removed:
            require(slug in RESTORE or True, "unreachable")
        seen.update(removed)
        planned.append((target, stripped, removed))

    missing = sorted(set(RESTORE) - seen)
    require(not missing,
            f"these rewritten sites were never found, so nothing was put back: {missing}")
