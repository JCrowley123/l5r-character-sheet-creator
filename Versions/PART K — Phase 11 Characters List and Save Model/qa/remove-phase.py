#!/usr/bin/env python3
"""Surgically remove PART K Phase 11 (Characters List and Save Model) from a SCRATCH COPY of the
Phase 0 tree.
...
lines, and the characters-seam and characters-init blocks in 210-test-seam-and-init.js are
removed. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 11"
FRAGMENTS = (
    "src/sheet/209.993-feat-characters-list.js",
    "src/css/59.993-feat-characters-list.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("characters-seam", "characters-init")}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "864c5134126ad8977bc39f764598aeda680c4db964bbafb8a755b88c9e5d4a04"
PRE_RELEASE_BYTES = 2928189
PRE_RELEASE_COMMIT = "a2312e4"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
# (?![.\d]) so a later Phase 11.1 or 11.2 marker is never mistaken for this one.
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+11(?![.\d])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 11 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END CHARLIST11 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+CHARLIST11\b", re.I)
SURFACE_RE = re.compile(r"\b(?:CHARACTERS_LIST_ENABLED|CHARACTERS_AUTOSAVE_ENABLED|CHARACTERS_SHARE_ENABLED|CL11|cl11[A-Za-z0-9_-]*|initCharactersList)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("characters-seam", "characters-init")) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END CHARLIST11")
