...
const previous = path.resolve(__dirname, '../../BUGFIX — Spell Slots Tab on Safari/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Spell Slots Tab on Safari/],
  [path.join(__dirname, 'characters-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 11 Characters List`);
