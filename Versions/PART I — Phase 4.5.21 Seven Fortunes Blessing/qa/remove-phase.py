#!/usr/bin/env python3
"""Surgically remove Seven Fortunes' Blessing from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact fortune-blessing-seam block are removed.
...
MARKER = "PART I FEATURE 4.5.21"
FRAGMENTS = (
    "src/sheet/209.9298-feat-adv-fortune-blessing.js",
    "src/css/59.97-adv-fortune-blessing.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("fortune-blessing-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "106833069f5bf72a69b185b9d29c85c7f0d1cbf6748bff2195a2581b5d1e8e79"
PRE_RELEASE_BYTES = 2820250
PRE_RELEASE_COMMIT = "5b1fc0082b576a6fb1f118a06bba119fe408c49e"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.21(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.21 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END FB4521 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+FB4521\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_FORTUNE_BLESSING_ENABLED|FB4521|fb4521[A-Za-z0-9_-]*|fortuneBlessing)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("fortune-blessing-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END FB4521")
