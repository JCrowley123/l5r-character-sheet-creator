/* Current regression entry point, as of Feature 4.56.
 *
 * Delegates to Feature 4.55's runner UNCHANGED — which in turn delegates to 4.5.2's — and adds
 * this release's own suite, reporting one combined total. When 4.56 is rolled back this file
 * goes with its folder and 4.55's runner is the entry point again.
 *
...
const previous = path.join(repo, 'Versions/PART I — Phase 4.5.5 Eligibility Gates/qa/current-suite-runner.js');
const own = path.join(__dirname, 'adv-rank-entries-harness.js');
...
const previousOutput = runOne('Feature 4.55 suite set', previous, [sheet]);
const trailer = /COMBINED (\d+)\/(\d+) checks passed/.exec(previousOutput);
if (trailer) { passed += Number(trailer[1]); total += Number(trailer[2]); }
else { console.log('FAIL the 4.55 runner produced no combined total'); failures += 1; }

const ownOutput = runOne('Feature 4.56 rank entries', own, [sheet]);
const ownCounts = /(\d+)\/(\d+) checks passed/.exec(ownOutput);
if (ownCounts) { passed += Number(ownCounts[1]); total += Number(ownCounts[2]); }
else { console.log('FAIL the 4.56 harness produced no count'); failures += 1; }

console.log(`\nCOMBINED ${passed}/${total} checks passed across every retained suite plus Feature 4.56`);
