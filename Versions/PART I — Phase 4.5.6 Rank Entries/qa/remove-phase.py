#!/usr/bin/env python3
"""Remove Part I Feature 4.56 (Rank Entries) from a COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY
    python qa/remove-phase.py COPY --expect-sha 0538c466...
...
Same safety model as Feature 4.54's and 4.55's removers, and for the same reason: explicit
...
PRE_RELEASE_SHA = "0538c4662e19ba633a3faefb8d1111f1b502f557e7f7b935c7fb53ae6570ab7f"
PRE_RELEASE_BYTES = 2533897
# The Feature 4.55 build this release was added to, after that phase's same-day Skill-picker
# revision. Removing 4.56 restores exactly that.
...
END_RE = re.compile(r"\bEND RANKS456\s+([a-z][a-z0-9-]*)\b")
...
def strip_owned_blocks(text: str, label: str) -> tuple[str, list[str]]:
    ...
    while index < len(lines):
        ...
        if not start:
            require(not END_RE.search(line), f"{label}:{index + 1}: orphan END RANKS456")
            output.append(line)
            index += 1
            continue
        slug = start.group(1)
        ...
        while index < len(lines):
            ...
            if end:
                require(end.group(1) == slug,
                        f"{label}:{index + 1}: END RANKS456 {end.group(1)} closes BEGIN {slug}")
                closed = True
                index += 1
                break
