#!/usr/bin/env python3
"""Surgically remove Seven Fortunes' Blessing (A01) from a SCRATCH COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY [--dry-run] [--expect-sha SHA256]

Preflights the complete operation before writing. Only the two owned fragments, their manifest
lines, and the exact blessing-seam block are removed. Later features are preserved; the resulting
manifest hash is calculated from the remaining fragments rather than forcing an old baseline.
Rebuild the copy with its build/recombine.py afterwards. No snapshot restore or live-tree edit
exists. Adapted from Feature 4.5.13's own qa/remove-phase.py (same shape, this phase's identity).
"""
...
MARKER = "PART I FEATURE 4.5.14"
FRAGMENTS = (
    "src/sheet/209.926-feat-adv-fortune-blessing.js",
    "src/css/59.9-adv-fortune-blessing.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("blessing-seam",)}
...
# Phase 0 build measured at 2,704,237 bytes / this SHA-256, both reproduced from that commit
# before writing this fragment.
PRE_RELEASE_SHA = "1e2683d81ddab13662f76c62ca8c595d521d633f6cc66a512169661cd85b8326"
PRE_RELEASE_BYTES = 2704237
PRE_RELEASE_COMMIT = "9dd3fef0c86e4494ef5ceb75a47e502ec78d0639"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+I\s+FEATURE\s+4\.5\.14(?![\d.])\b", re.I)
BEGIN_RE = re.compile(r"^\s*// PART I FEATURE 4\.5\.14 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END BLESSING4514 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+BLESSING4514\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_FORTUNE_BLESSING_ENABLED|FB4514|fortuneBlessing4514[A-Za-z0-9_-]*)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("blessing-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END BLESSING4514")
        ...
        # Marker-shaped prose counts too. This deliberately rejects even BUGFIX in prose.
        if active:
