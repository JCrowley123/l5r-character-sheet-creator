  ...
  const CW1122 = (function(){
    ...
    // The sheet's own picker groups each category word stands for. Weapon Skills are Bugei
    // Skills, so "Bugei" offers the Weapon group; the Weapon (Low) group (Cannon, Firearms,
    // Ninjutsu) only when the choice also says Low or Weapon (the owner's ruling, 25 September).
    const GROUPS = {
      high: ['High'],
      bugei: ['Bugei', 'Weapon'],
    ...
    // What "non-X" or "(not X)" leaves out: every group of that kind, Weapon (Low) included.
    const EXCLUDE = {
