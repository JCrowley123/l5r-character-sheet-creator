  ...
  // "Kenjutsu Rank 3" / "Ninjutsu Ranks 3, 7" / "Kenjutsu" when the granting rank is not knowable.
  // The bare-name form is not a fallback to the old behaviour: it drops the rank claim entirely
  // rather than naming one that might be wrong, and the sentence it prefixes still reads correctly
  // because every line this fix touches continues with either " mastery ..." or ": ...".
  function masteryRankPhrase(skillName, ranks){
    if(!ranks || !ranks.length) return skillName;
    if(ranks.length === 1) return skillName + ' Rank ' + ranks[0];
    return skillName + ' Ranks ' + ranks.join(', ');
  }
  ...
  function masteryRankLabelRewrite(breakdown, from, ctx){
    ...
    for(let i=start; i<breakdown.length; i++){
      ...
      const rest = line.slice(oldPrefix.length);
      let ranks;
      if(rest.indexOf(' mastery +') === 0)                      ranks = masteryContributingRanks(raw.dmgBonus, ctx.skillRank);
