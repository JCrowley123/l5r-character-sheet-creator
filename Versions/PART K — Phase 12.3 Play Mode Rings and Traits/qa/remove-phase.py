#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12.3 Play Mode Rings and Traits (part 4) from a SCRATCH COPY of the Phase 0 tree.

    python qa/remove-phase.py COPY [--dry-run] [--expect-sha SHA256]

Preflights the complete operation before writing. Only the owned fragment and stylesheet, their
manifest lines, and the modes-rings-traits-seam block in 210-test-seam-and-init.js are removed. The
Rings & Traits tab goes back to editing in both modes. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12.3"
FRAGMENTS = (
    "src/sheet/209.9999-feat-modes-rings-traits.js",
    "src/css/59.9991-feat-modes-rings-traits.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-rings-traits-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "c4ddd6fbd9cdc6bd743509ff93936673996922c2956237bb17b8dad27ceae932"
PRE_RELEASE_BYTES = 3088014
PRE_RELEASE_COMMIT = "7ea0b3c"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12\.3(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12\.3 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES123 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES123\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES123_ENABLED|MODES123)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-rings-traits-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES123")
