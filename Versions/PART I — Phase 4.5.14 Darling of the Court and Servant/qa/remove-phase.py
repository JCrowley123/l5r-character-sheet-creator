#!/usr/bin/env python3
"""Surgically remove Darling of the Court and Servant from a SCRATCH COPY of the Phase 0 tree.
...
their manifest lines, and the exact court-servant-seam block are removed.
...
FRAGMENTS = (
    "src/sheet/209.926-feat-adv-court-servant.js",
    "src/css/59.9-adv-court-servant.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("court-servant-seam",)}
...
END_RE = re.compile(r"^\s*// END CS4514 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+CS4514\b", re.I)
SURFACE_RE = re.compile(r"\b(?:ADV_COURT_SERVANT_ENABLED|CS4514|cs4514[A-Za-z0-9_-]*|court4514[A-Za-z0-9_-]*|servant4514[A-Za-z0-9_-]*|courtList)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("court-servant-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END CS4514")
