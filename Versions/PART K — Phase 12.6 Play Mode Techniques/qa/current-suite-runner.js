...
const previous = path.resolve(__dirname, '../../PART K — Phase 12.4 Play Mode Skills/qa/current-suite-runner.js');
let passed = 0, total = 0, failures = 0;
for (const [file, pattern] of [[previous, /COMBINED (\d+)\/(\d+) checks passed across every retained suite plus Phase 12\.4 \(part 5\) Skills/],
  [path.join(__dirname, 'techniques-harness.js'), /(\d+)\/(\d+) checks passed/]]) {
...
console.log(`COMBINED ${passed}/${total} checks passed across every retained suite plus Phase 12.6 Techniques`);
