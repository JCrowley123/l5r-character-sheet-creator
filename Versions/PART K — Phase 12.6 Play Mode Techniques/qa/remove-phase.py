#!/usr/bin/env python3
"""Surgically remove PART K — Phase 12.6 Play Mode Techniques (the Techniques part) from a SCRATCH COPY of the Phase 0 tree.
...
manifest lines, and the modes-techniques-seam block in 210-test-seam-and-init.js are removed. The
Techniques tab goes back to editing in both modes. Later features are preserved; the resulting manifest hash is calculated from the
...
MARKER = "PART K PHASE 12.6"
FRAGMENTS = (
    "src/sheet/209.99992-feat-modes-techniques.js",
    "src/css/59.9993-feat-modes-techniques.css",
)
SHARED_BLOCKS = {"src/sheet/210-test-seam-and-init.js": ("modes-techniques-seam",)}
SHARED_FILES = tuple(SHARED_BLOCKS)
PRE_RELEASE_SHA = "4ac2b09f5ba7f3c2945bcce71f0f64e1fdc79a4d1d92754eb5aea2d66cf8adc1"
PRE_RELEASE_BYTES = 3093125
PRE_RELEASE_COMMIT = "adb65d6"
MARKER_RE = re.compile(r"\b(PART\s+[A-Z]+\s+(?:PHASE|FEATURE)\s+\d+(?:\.\d+)*|BUGFIX)\b", re.I)
OWN_RE = re.compile(r"\bPART\s+K\s+PHASE\s+12\.6(?![\d.])", re.I)
BEGIN_RE = re.compile(r"^\s*// PART K PHASE 12\.6 BEGIN ([a-z][a-z0-9-]*)\s*$")
END_RE = re.compile(r"^\s*// END MODES126 ([a-z][a-z0-9-]*)\s*$")
OWN_END_RE = re.compile(r"\bEND\s+MODES126\b", re.I)
SURFACE_RE = re.compile(r"\b(?:MODES126_ENABLED|MODES126)\b")
...
def strip_owned_blocks(text: str, label: str,
                       expected: tuple[str, ...] = ("modes-techniques-seam",)) -> tuple[str, list[str]]:
    output, removed = [], []
    ...
    for number, line in enumerate(lines, 1):
        ...
        if begin:
            ...
            continue
        if end:
            require(active is not None, f"{label}:{number}: orphan END MODES126")
